...
impl Runtime {
    ...
    pub fn run_frames(&self, frames: usize) {
        ...
        if self.tile_progress.load(Ordering::SeqCst) >= total_tiles {
            return;
        }

        let num_jobs = frames * self.tiles_per_frame;
        for _ in 0..num_jobs {
    ...
    pub fn deserialize_checkpoint(bytes: &[u8]) -> Self {
        let binary = decompress_size_prepended(bytes)
            ...
            .unwrap();
        let ron = String::from_utf8(binary)
            .map_err(
                |e| log::error!(target: "Runtime", "unable to parse checkpoint to String: {}", e),
            )
            .unwrap();
        ron::from_str::<RuntimeSerde>(&ron)
...
impl RuntimeSerde {
    pub fn save_to<P: AsRef<Path>>(&self, path: P) {
        ...
        let ser = ron::ser::to_string_pretty(&self, Default::default())
