...
impl From<Config> for OrthographicCamera {
    fn from(c: Config) -> Self {
        ...
        let top_left = c.position - 0.5 * (c.fov.x as Float * x_unit + c.fov.y as Float * y_unit);

        let x_dir = c.fov.x as Float / c.resolution.x as Float * x_unit;
        let y_dir = c.fov.y as Float / c.resolution.y as Float * y_unit;
